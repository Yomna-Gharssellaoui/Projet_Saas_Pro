...
export class SupportChatService {
  constructor(
    ...
    // Log AI config at startup
    const key = process.env.GROQ_API_KEY || "";
    const model = process.env.GROQ_MODEL || "llama-3.1-8b-instant";
    console.log(`[SupportChatService] GROQ_API_KEY present: ${!!key}, length: ${key.length}, starts with gsk_: ${key.startsWith("gsk_")}`);
    console.log(`[SupportChatService] GROQ_MODEL: ${model}`);
    console.log(`[SupportChatService] useGroq will be: ${!!key && key !== "your_groq_api_key_here" && key.startsWith("gsk_")}`);
  }
  ...
  private get useGroq()  {
    const k = this.groqKey;
    const valid = !!k && k !== "your_groq_api_key_here" && k.startsWith("gsk_");
    console.log(`[useGroq] key present: ${!!k}, valid: ${valid}, key prefix: ${k.substring(0, 8)}...`);
    return valid;
  }

  /**
   * Check if AI service is available
   * If Groq key is configured → always available (no local check needed)
   */
  private async isOllamaAvailable(): Promise<boolean> {
    if (this.useGroq) {
      console.log("[isOllamaAvailable] Groq configured → returning true");
      return true;
    }
    console.log("[isOllamaAvailable] No Groq key — checking local Ollama...");
    try {
      ...
      try {
        ...
        console.log(`[isOllamaAvailable] Ollama response: ${response.status}`);
        return response.ok;
      } finally {
        clearTimeout(timeoutId);
      }
    } catch (e: any) {
      console.error("[isOllamaAvailable] Ollama not reachable:", e.message);
      return false;
    }
  }
  async closeOpenTickets(businessId: string, userId: string) {
    // Close both "open" and "in_progress" tickets so new conversation starts clean
    await this.ticketRepo
      .createQueryBuilder()
      .update(SupportTicketEntity)
      .set({ status: "closed" as any, resolvedAt: new Date() })
      .where("businessId = :businessId AND userId = :userId AND status IN (:...statuses)", {
        businessId,
        userId,
...
Entreprise: ${businessName}`;
  ...
  private async callOllama(
    ...
    console.log(`[callOllama] useGroq=${this.useGroq}, groqKey length=${this.groqKey.length}, model=${this.groqModel}`);
    if (this.useGroq) {
      console.log(`[callOllama] → Using Groq API with model: ${this.groqModel}`);
      return this.callGroq(messages, systemPrompt);
    }
    console.log(`[callOllama] → Using local Ollama with model: ${OLLAMA_MODEL}`);
  ...
  private async callGroq(
    ...
    try {
      console.log(`[Groq] Calling API, key: ${this.groqKey.substring(0, 12)}..., model: ${this.groqModel}`);
      ...
      if (!response.ok) {
        ...
        // Return a helpful message instead of crashing or falling back to unavailable Ollama
        if (response.status === 401) {
          return "⚠️ Clé API Groq invalide. Veuillez vérifier la configuration.";
        }
        if (response.status === 429) {
          return "⏳ Limite de requêtes Groq atteinte. Veuillez réessayer dans quelques secondes.";
        }
        return `Désolé, le service IA est temporairement indisponible (erreur ${response.status}). Veuillez réessayer.`;
      }
      ...
      // Do NOT fall back to Ollama — just return a user-friendly message
...
- Conseils pour récupérer les factures impayées`;
      ...
      // Reload ticket messages from DB to get the freshly saved user message
      const freshTicket = await this.ticketRepo.findOne({
        where: { id: ticket.id },
        relations: ["messages"],
      });

      const conversationMessages = (freshTicket?.messages ?? [])
  ...
  async getUserTickets(businessId: string, userId: string) {
    ...
    // Sort messages within each ticket by createdAt ASC
    tickets.forEach((t) => {
      if (t.messages) {
        t.messages.sort(
  ...
  /**
   * Get a single ticket by ID for admin (no businessId restriction)
   * Returns full ticket with all messages sorted by createdAt ASC
   */
   ...
   * Returns ALL escalated tickets across ALL businesses, sorted by most recent
   */
  ...
  async adminReplyByTicketId(ticketId: string, dto: AdminReplyDto) {
    ...
    // Enrich with owner info for admin UI
    let ownerName = "Client inconnu";
    let ownerEmail = "";
    let businessName = "";
    try {
